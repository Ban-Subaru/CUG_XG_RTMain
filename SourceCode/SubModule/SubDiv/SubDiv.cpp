...
public:
	...
	cSubDivApp() :
		cApplication()
	{
		m_title = wstring( L"Terrain rendering Sample(By Subaru)" );
		m_pSurf = NULL;
		FileName = L"";
...
void cSubDivApp::SceneInit()
{
	...
	SetFileName(L"..\\..\\OutPut\\Data\\MEDIA\\atestterrain.o3d");
	...
	m_timeDelta = 0.003f; // a 30th of a second is a good first value
...
void cSubDivApp::SetFileName(std::wstring filename)
{
	m_filename = filename;
}

bool cSubDivApp::IsEmptyFileName()
{
	return m_filename == L"" ? false : true;
}

void cSubDivApp::InitLights()
{
	LightControl _ligcon;
	_ligcon.SetLightColor();

	float_3 *_ptr = _ligcon.GetLightColor();
	Graphics()->AddLight(D3DXCOLOR(_ptr[1].r,_ptr[1].g, _ptr[1].b, 1), D3DXVECTOR3(0, 1, 0));

	//Graphics()->AddLight(D3DXCOLOR(0.5, 0.5, 0, 1), D3DXVECTOR3(0,0,1));
	//Graphics()->AddLight(D3DXCOLOR(0.5, 0.5, 0, 1), D3DXVECTOR3(0, 0, -1));
}
...
void cSubDivApp::KeyUp( int key )
{
	...
	if( key == DIK_C )
	{
		...
		{
			pDevice->RSSetState(m_pWireState);  //设置面着色
		}
		else 
		{
			pDevice->RSSetState(m_pSolidState);//设置线着色
		}	
