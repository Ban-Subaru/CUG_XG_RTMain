...
eResult cSubDivSurf::Subdivide()
{
	...
	for( i=0; i<m_nTris; i++ )
	{
		...
		if (m_pTList[i].m_v[0]->GetEdge(m_pTList[i].m_v[1]) == NULL ||
			m_pTList[i].m_v[1]->GetEdge(m_pTList[i].m_v[2]) == NULL ||
			m_pTList[i].m_v[2]->GetEdge(m_pTList[i].m_v[0]) == NULL
			)
		{
			continue;
		}
...
point3 cSubDivSurf::sEdge::CalcKVert(int prim, int sec)
{
		...
		//assert( false );
	{
		...
		if (pOther != NULL)
		    out += (-0.9f / 8.f) * *(point3*)(&pOther->m_vert.m_vPosition);
		else
			out += (-0.9f / 8.f) * *(point3*)(&m_v[sec]->m_vert.m_vPosition);
	}
...
void cSubDivSurf::GenD3DData()
{
    ...
    if(!m_pVertexBuffer)
	{
       assert(L"Vertex Buffer creation failed!\n");
	}
