...
class cSubDivSurf  
{
	...
	struct sVert
	{
		...
		sEdge*	GetEdge( sVert* pOther )
		{
			...
			//return NULL;
			if ((int)m_edgeList.size() > 0)
			{
				return m_edgeList[0];//(int)m_edgeList.size() - 1
			}
			else
				return NULL;
		}
	...
	struct sEdge
	{
		...
		{
			sTriangle* pTri = NULL;
			for( int i=0; i<(int)v0->m_triList.size(); i++ )
			{
				pTri = v0->m_triList[i];
			...
			//assert(false);
			//pTri = v0->m_triList[(int)v0->m_triList.size() - 1];
			if (v0->m_triList.size() != 0 && pTri != NULL )
			{
				return pTri->Other(v0, v1);
			}
			else
...
public:
	...
	cSubDivSurf( const TCHAR* filename );
	virtual ~cSubDivSurf();
