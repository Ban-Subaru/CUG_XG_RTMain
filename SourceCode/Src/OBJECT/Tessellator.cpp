...
void Tessellator::beginTessellation()
{
	...
	gluTessCallback(_tobj, GLU_TESS_VERTEX_DATA, (void(__stdcall *)()) vertexCallback);
	gluTessCallback(_tobj, GLU_TESS_BEGIN_DATA, (void(__stdcall *)()) beginCallback);
	gluTessCallback(_tobj, GLU_TESS_END_DATA, (void(__stdcall *)()) endCallback);
	gluTessCallback(_tobj, GLU_TESS_COMBINE_DATA, (void(__stdcall *)()) combineCallback);
	gluTessCallback(_tobj, GLU_TESS_ERROR_DATA, (void(__stdcall *)()) errorCallback);
...
void Tessellator::retessellatePolygons(osg::Geometry &geom)
{
    ...
    {
        PrimitiveSet* primitive = _Contours[primNo];
        ...
        { // this recovers the 'old' tessellation which just retessellates single polygons.
            if (primitive->getMode()==osg::PrimitiveSet::POLYGON || _ttype==TESS_TYPE_DRAWABLE)
            {
                ...
                {
                    osg::DrawArrayLengths* drawArrayLengths = static_cast<osg::DrawArrayLengths*>(primitive);
...
void Tessellator::handleNewVertices(osg::Geometry& geom,VertexPtrToIndexMap &vertexPtrToIndexMap)
{
    if (!_newVertexList.empty())
    {
            ...
            ++tcalItr)
        {
            if (tcalItr->array) 
            {
                arrays.push_back(tcalItr->array);
            }
        }
